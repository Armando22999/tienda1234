  	...
  	$datosJson = '
	 	...
	 	for($i = 0; $i < count($productos); $i++){
			...
			if(is_array($categorias) && $categorias["categoria"] == ""){
				...
				$categoria =  is_array($categorias) && ["categoria"];
			}
			...
			if($productos[$i]["oferta"] != 0){
  			...
  			$acciones = "<div class='btn-group'><button class='btn btn-warning btnEditarProducto' idProducto='".$productos[$i]["id"]."' data-toggle='modal' data-target='#modalEditarProducto'><i class='fa fa-pencil'></i></button><button class='btn btn-danger btnEliminarProducto' idProducto='".$productos[$i]["id"]."' imgOferta='".$productos[$i]["imgOferta"]."' rutaCabecera='".$productos[$i]["ruta"]."'imgPortada='" . (is_array($cabeceras) ? $cabeceras['portada'] : 'vistas/img/cabeceras/default/default.jpg') ."' imgPrincipal='".$productos[$i]["portada"]."'><i class='fa fa-times'></i></button></div>";
